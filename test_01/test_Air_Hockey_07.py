...
def draw_boundaries():
    ...
    pygame.draw.line(screen, WHITE, [window_width / 2 + 100, edge], [window_width / 2 + 100, window_height - edge], 5)
    pygame.draw.circle(screen, WHITE, (0, window_height // 2), 200, 4)
    pygame.draw.circle(screen, WHITE, (window_width, window_height // 2), 200, 5)  # Großer Kreis
...
while running:
    ...
    for i, joystick in enumerate(joysticks):
        ...
        players[i][0][0] = max(player_radius + 10, min(players[i][0][0], window_width - player_radius - 10))
        players[i][0][1] = max(player_radius + 10 + edge, min(players[i][0][1], window_height - player_radius - 10 - edge))
    ...
    # ball_x = max(ball_radius + 10, min(ball_x, window_width - ball_radius - 10))
    # ball_y = max(ball_radius + 10 + edge, min(ball_y, window_height - ball_radius - 10 - edge))

    if ball_x - ball_radius < ball_radius + 10:
        ball_x = ball_radius + 10
        ball_velocity_x *= -1
    elif ball_x + ball_radius > window_width - ball_radius - 10:
        ball_x = window_width - ball_radius - 10
        ball_velocity_x *= -1
    if ball_y - ball_radius < ball_radius + 10 + edge:
        ball_y = ball_radius + 10 + edge
        ball_velocity_y *= -1
    elif ball_y + ball_radius > window_height - ball_radius - 10 - edge:
        ball_y = window_height - ball_radius - 10 - edge
        ball_velocity_y *= -1
